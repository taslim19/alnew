...
• Perintah: <code>{0}info</code> [user_id/username/reply to users]
• Penjelasan: Untuk mendapatkan info pengguna telegram dengan deskripsi lengkap.

• Perintah: <code>{0}cinfo</code> [chat_id/username/reply to chat]
• Penjelasan: Untuk mendapatkan info group/channel dengan deskripsi lengkap.
"""


@ubot.on_message(filters.me & anjay("info"))
async def _(client, message):
    user_id = await extract_user(message)
    Tm = await message.reply("</code>Processing . . .</code>")
    if not user_id:
        return await Tm.edit(
            "<code>Berikan userid/username/reply untuk mendapatkan info pengguna tersebut.</code>"
        )
    try:
...
@ubot.on_message(filters.me & anjay("cinfo"))
async def _(client, message):
    Tm = await message.reply("</code>Processing . . .</code>")
    try:
